-- Derived from tabby `FileNameResolver`

---@class FileNameLoader load names for lazy loading
---@field get_name fun(handler:number):string get name by handler
---@field get_names fun():table<number, string> get all names

---@alias FileNameMode 'unique'|'relative'|'tail'|'shorten'

---@class FileNameResolver
---@field private loader FileNameLoader load names for lazy loading
...
local function relative(name) return vim.fn.fnamemodify(name, ":~:.") end

local function head(name) return vim.fn.fnamemodify(name, ":h") end

local function tail(name) return vim.fn.fnamemodify(name, ":t") end
...
function FileNameResolver:uniquify_names(fallback)
  ---@alias FileNameIndexItem {h: string, n:number}
  ---@alias FileNameIndex table<string, FileNameIndexItem[]>
  local all_names = self.loader.get_names()
  local indexes = {} ---@type FileNameIndex

  for handler, name in pairs(all_names) do
    if name == nil or name == "" then name = fallback(handler) end
    name = relative(name)
  ...
  local next_index = {} ---@type FileNameIndex
      ...
      if #items == 1 then
        ...
        for _, item in ipairs(items) do
          ...
          if hh == "." or hh == "/" then
            ...
            table.insert(next_index[name], { h = head(hh), n = item.n })
          end
        end
      end
    end
...
local default_fallback = function(_) return "" end

---Get name of a handler
---@param handler number
---@param mode FileNameMode
---@param fallback? fun(handler:number):string
---@return string
function FileNameResolver:get_name(handler, mode, fallback)
  ...
  if mode == "unique" then
    ...
    return self:get_other_name(
      handler,
      mode --[[@as 'relative'|'tail'|'shorten']],
      fallback
    )
  end
end
